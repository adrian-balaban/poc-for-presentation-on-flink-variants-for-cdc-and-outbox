...
class CheckpointConfigurerTest {
    ...
    void applyExactlyOnce_setsAllCheckpointParameters() {
        ...
        assertEquals(CheckpointConfigurer.MAX_CONCURRENT_CHECKPOINTS, cfg.getMaxConcurrentCheckpoints());
        assertEquals(CheckpointConfigurer.CHECKPOINT_TIMEOUT_MS, cfg.getCheckpointTimeout());
        assertEquals(CheckpointConfigurer.MIN_PAUSE_BETWEEN_CHECKPOINTS_MS, cfg.getMinPauseBetweenCheckpoints());
    }
}
