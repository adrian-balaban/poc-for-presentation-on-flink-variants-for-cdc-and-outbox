...
- Do not pass unqualified image names to `kind load docker-image` when images were built with Podman. Podman stores locally-built images under the `localhost/` registry prefix (e.g. `localhost/flink-with-mysql:latest`); passing the bare name `flink-with-mysql:latest` causes kind to report `ERROR: image: "flink-with-mysql:latest" not present locally` even though the image exists. Always use `localhost/<name>:<tag>` with `kind load docker-image`. The subsequent `ctr` re-tag block in `deploy.sh` already sources from `localhost/<name>:latest` so no change is needed there.
