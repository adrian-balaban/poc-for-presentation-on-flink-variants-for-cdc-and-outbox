 ...
 *
 * Validates job behavior under failure conditions: MySQL connectivity issues,
 * Kafka unavailability, and recovery semantics.
 */
@Slf4j
...
class ErrorScenarioTest extends FlinkTestBase {
    ...
    void jobContinuesAfterTransientMysqlDelay() throws Exception {
        ...
        log.info("Job recovered after delay: received {} messages",
    ...
    void multipleConsecutiveInserts_doNotProduceDuplicates() throws Exception {
        ...
        int countDupTest0 = (int) messages.stream()
            .filter(m -> m.contains("DUP-TEST-0"))
            .count();

        assertThat(countDupTest0).as("Should not have duplicate messages for same row")
    ...
    void jobStaysBusyUnderLoadAndRecoverable() throws Exception {
        ...
        log.info("Job recovered after load burst: {} messages total", messages.size() + afterMessages.size());
    }
