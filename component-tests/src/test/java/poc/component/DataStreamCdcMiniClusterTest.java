 ...
 *
 * Fast feedback during development without needing external services.
 * Tests core CDC transformation: event enrichment and topic routing.
 *
 * For full end-to-end testing with MySQL CDC source and Kafka sink, use DataStreamCdcTest.
 */
@Slf4j
...
class DataStreamCdcMiniClusterTest extends MiniClusterTestBase {
    ...
    void cdcRouter_enrichesEventWithVariantAndTopic() throws Exception {
        ...
        assertThat(collector.out).allMatch(
            msg -> msg.contains("\"variant\":\"datastream-cdc\"")
        );
        assertThat(collector.out).allMatch(
            msg -> msg.contains("\"topic\":\"poc.cdc.datastream\"")
        );
