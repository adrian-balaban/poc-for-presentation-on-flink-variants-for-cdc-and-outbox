 ...
 *
 * <p>Tests dynamic topic routing based on the `destination` field in outbox events. For full
 * end-to-end testing with real CDC source and Kafka, use DataStreamOutboxTest.
 */
@Slf4j
...
class OutboxRouterMiniClusterTest extends MiniClusterTestBase {
  ...
  void outboxRouter_extractsDestinationField() throws Exception {
    ...
    String dest1 =
        router.extractField("{\"destination\":\"payments\",\"amount\":100}", "destination");
    String dest2 =
        router.extractField("{\"destination\":\"audit\",\"event\":\"login\"}", "destination");
  ...
  void outboxRouter_handlesEscapedQuotes() throws Exception {
    ...
    String field = router.extractField("{\"destination\":\"pay\\\"ments\"}", "destination");
